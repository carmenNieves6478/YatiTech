...
export const PersonalNotes: React.FC<Props> = ({ userId, lessonId }) => {
  ...
  return (
    <Card className="bg-white border-slate-200 p-5 space-y-4 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          {savingStatus === "error" && (
            <span className="text-red-600 text-[11px] font-semibold">Error al guardar</span>
          )}
        </div>
      </div>
      ...
      <div className="space-y-3">
        <textarea
          ...
          className="w-full bg-slate-50 border border-slate-300 rounded-xl p-3.5 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-teal-600 focus:ring-1 focus:ring-teal-600 leading-relaxed resize-y"
        />
